  ...
  if vLocale = 'NZ' then
    if ctr.ctr_event is not null and ctr.ctr_duplicate ='-' then
		...
		if vCount > 0 then
		    ...
		    /*--to fixed error COCT Remark is not copy to WO Comment. Databridge is not supported field Copy to WO Commnt for Remarks.
		    if ctr.ctr_note is not null then	  
			  begin
				...
				where add_text like ctr.ctr_note||'%';
			   ...
			   exception when no_data_found then
				 ...
				 values
				 ('EVNT','EVNT','*','*',ctr.ctr_event,'EN',vAddLine,'+',ctr.ctr_note,o7gttime(ctr.ctr_org));
			   end;
		    end if;--ctr.ctr_note is not null then
			*/
