...
begin
  select * into par from r5parts where rowid=:rowid;
   ...
   /**Validate same sap code***/
   select count(1) into vCnt from r5parts 
   where par_udfchar20 = par.par_udfchar20
   and   par_org  ='CAUS';
   if vCnt > 1 then 
      iErrMsg := 'Duplidate SAP Code is found in system, please check.';
      raise err_validate;
   end if;
...
when others then
  RAISE_APPLICATION_ERROR (-20001,'Error in Flex R5PARTS/Post Insert/200');
