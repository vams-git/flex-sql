...
begin
  ...
  if evt.evt_jobtype not in ('MEC') and evt.evt_org IN ('TAS','RRM','VIC','WAU','WAR','NWA','SAU','NSW','QLD','NTE','NVE','NVW','NVP','FCG') then
     ...
     end if; --evt.evt_status in ('40PR') and evt.evt_person is not null
  end if; --evt.evt_jobtype not in ('MEC') and evt.evt_org IN ('TAS','RRM','NVE','NVW','NVP') 
  
exception 
 when test_err then 
   RAISE_APPLICATION_ERROR (-20003,'ERR/R5EVENTS/290/I - '||iErrMsg);  
 when others then       
   RAISE_APPLICATION_ERROR (-20001,'ERR/R5EVENTS/290/I - ');   
end;
