...
begin
  ...
  /*****Copy work order document to requistion*****/
  if rql.rql_event is not null then
     ...
     select dae_document,'REQ','REQ',dae_type,dae_rtype,rql.rql_req,
     dae_copytowo,dae_printonwo,dae_copytopo,dae_printonpo,dae_createcopytowo
     from r5docentities
     where dae_entity = 'EVNT'
     and   dae_code = rql.rql_event
     and   dae_document not in 
     (select dae_document from r5docentities where dae_entity = 'REQ' and dae_code = rql.rql_req);
  end if;
